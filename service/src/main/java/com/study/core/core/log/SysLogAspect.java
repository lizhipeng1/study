...
import cn.xluobo.business.sys.log.service.ISysOperateLogService;
import cn.xluobo.config.login.LoginUser;
import cn.xluobo.core.utils.ServletUtils;
import cn.xluobo.core.utils.ip.IpUtils;
...
/**
 * @author ：zhangbaoyu
 * @date ：Created in 2021/4/20 19:29
 */
@Aspect
@Component
@Slf4j
public class SysLogAspect {
    ...
    // 配置织入点
    @Pointcut("@annotation(cn.xluobo.core.log.Log)")
    ...
    protected void handleLog(final JoinPoint joinPoint, final Exception e, Object jsonResult) {
        try {
            ...
            // 保存数据库
            operateLogService.asyncSaveLog(operLog);
        } catch (Exception exp) {
            // 记录本地异常日志
            log.error("==前置通知异常==");
    ...
    public void getControllerMethodDescription(JoinPoint joinPoint, Log log, SysOperateLog operLog) throws Exception {
        ...
        // 设置标题
        operLog.setOperateModule(log.module().getValue());
        // 设置操作人类别
        operLog.setClientType(log.clientType().getValue());
        // 是否需要保存request，参数和值
    ...
    private void setRequestValue(JoinPoint joinPoint, SysOperateLog operLog) throws Exception {
        ...
        if (HttpMethod.PUT.name().equals(requestMethod) || HttpMethod.POST.name().equals(requestMethod)) {
            ...
            operLog.setReqParam(StringUtils.substring(paramsMap.toString(), 0, 2000));
        }
    }
