...
<script>
import { listReceipt, getReceipt, delReceipt, addReceipt, updateReceipt } from '@/api/system/receipt'
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      this.reset()
      getReceipt(row.accountId || this.ids).then(response => {
    ...
    submitForm: function() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          ...
          if (this.form.accountId !== undefined) {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const id = row.accountId || this.ids
      this.$confirm('是否确认删除?', '警告', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(function() {
        return delReceipt(id)
      }).then((response) => {
        if (response.respCode === '0000') {
          this.getList()
